...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $data = [
        ...
        'motiv_modificare' => $_POST['motiv_modificare']
    ];

    // Validări
    if (empty($data['numar_camion']) || empty($data['nume_sofer']) || empty($data['prenume_sofer']) || empty($data['numar_document'])) {
        $error = "Toate câmpurile obligatorii trebuie completate!";
    } elseif (empty($data['motiv_modificare'])) {
        $error = "Trebuie să specificați motivul modificării!";
    } else {
        // Actualizează datele și marchează ca modificate
        $query = "UPDATE atribuiri 
                 SET numar_camion = :numar_camion,
                     ...
                     date_modificate = 1,
                     motiv_modificare = :motiv_modificare,
                     data_modificare = NOW()
                 WHERE id = :id";
        ...
        $stmt->bindParam(":motiv_modificare", $data['motiv_modificare']);
        $stmt->bindParam(":id", $atribuire_id);
        
        if ($stmt->execute()) {
            $success = "Datele au fost actualizate cu succes! Managerul va fi notificat despre modificări.";
        } else {
            $error = "Eroare la actualizarea datelor!";
        }
    }
}
?>
...
<head>
    ...
    <title>Modificare Date Transport - Sistem Logistic</title>
    <style>
        * { 
...
<body>
    <div class="header">
        <h1>🔄 MODIFICARE DATE TRANSPORT</h1>
        <div class="user-info">
    ...
    <div class="container">
        ...
        <div class="info-section">
            <h3>📦 EXPEDIE #<?php echo $expeditie['id']; ?></h3>
            <div class="info-grid">
                <div class="info-item">
                ...
                <div class="info-item">
                    <span class="info-label">PRET CONTRACTAT:</span>
        ...
        <?php if ($success): ?>
            <div class="success">
                ...
                <br><small>Managerul va verifica noile date și va actualiza în sistemul de operare.</small>
            </div>
        <?php endif; ?>
        ...
        <!-- Formular modificare date -->
        <div class="form-container">
            <h2>📝 ACTUALIZARE DATE CAMION ȘI ȘOFER</h2>
            
            <div class="warning-box">
                ⚠️ <strong>ATENȚIE:</strong> Utilizați acest formular doar dacă:<br>
                1. Camionul inițial are o defecțiune<br>
                2. Șoferul inițial este indisponibil (îmbolnăvit, etc.)<br>
                3. Trebuie să schimbați remorca<br>
                <strong>Managerul va fi notificat despre modificări!</strong>
            </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" id="numar_camion" name="numar_camion" required
                               value="<?php echo htmlspecialchars($atribuire['numar_camion'] ?? ''); ?>"
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="numar_remorca" name="numar_remorca"
                               value="<?php echo htmlspecialchars($atribuire['numar_remorca'] ?? ''); ?>"
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" id="nume_sofer" name="nume_sofer" required
                               value="<?php echo htmlspecialchars($atribuire['nume_sofer'] ?? ''); ?>"
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="prenume_sofer" name="prenume_sofer" required
                               value="<?php echo htmlspecialchars($atribuire['prenume_sofer'] ?? ''); ?>"
                               placeholder="ex: Ion">
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select id="tip_document" name="tip_document" required>
                            ...
                            <option value="CI" <?php echo (($atribuire['tip_document'] ?? '') === 'CI') ? 'selected' : ''; ?>>Carte de Identitate (CI)</option>
                            <option value="PASAPORT" <?php echo (($atribuire['tip_document'] ?? '') === 'PASAPORT') ? 'selected' : ''; ?>>Pașaport</option>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="numar_document" name="numar_document" required
                               value="<?php echo htmlspecialchars($atribuire['numar_document'] ?? ''); ?>"
                               placeholder="ex: AX123456" maxlength="20">
                    </div>
                </div>

                <div class="form-group">
                    <label for="motiv_modificare" class="required">Motivul modificării</label>
                    <textarea id="motiv_modificare" name="motiv_modificare" rows="3" required
                              placeholder="ex: Camionul inițial are defect la motor, înlocuit cu alt camion..."><?php echo htmlspecialchars($_POST['motiv_modificare'] ?? ''); ?></textarea>
                </div>

                <button type="submit" class="btn" onclick="return confirm('Sigur doriți să modificați datele? Managerul va fi notificat și va trebui să reexporte datele către operare.')">
                    🔄 ACTUALIZEAZĂ DATELE
                </button>
            </form>
        </div>
    ...
    <script>
        ...
        document.querySelector('form').addEventListener('submit', function(e) {
            const motiv = document.getElementById('motiv_modificare').value;
            if (!motiv.trim()) {
                alert('Trebuie să specificați motivul modificării!');
                e.preventDefault();
                return;
            }
            
            if (!confirm('Sigur doriți să modificați datele? Managerul va fi notificat și va trebui să reexporte datele către operare.')) {
                e.preventDefault();
            }
        });
    </script>
